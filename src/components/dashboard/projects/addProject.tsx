import { addProject, assignProjectImages } from "@/api/projects";
...
const AddProjectModal: React.FC<AddProjectModalProps> = () => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      const createdProject = (response.data.project as Project);

      // Now, upload the images
      if (createdProject.id) {
        const formValues: z.infer<typeof formSchema> = form.getValues();
        uploadImages(createdProject.id, formValues.projectImages);
      }
    },
    onError: () => {
    ...
    <>
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild className="flex justify-center items-center">
          <Button className="w-full md:w-fit" variant={"default"}>Add Project</Button>
        </DialogTrigger>
        <DialogPortal>
